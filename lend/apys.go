...
import (
	...
	"github.com/0xVanfer/ethprotocol/requests"
	...
	if p.IsAaveLike {
		...
		case ethaddr.AaveV2Protocol:
			...
			for _, reserves := range aaveGraphs {
				...
				aTotalSupply, err := p.AToken.Basic.GetTotalSupply()
				if err != nil {
					return err
				}
				aPrice, err := p.AToken.UnderlyingBasic.GetPrice(gecko)
				if err != nil {
					return err
				}
				...
				vTotalSupply, err := p.VToken.Basic.GetTotalSupply()
				if err != nil {
					return err
				}
				vPrice, err := p.VToken.UnderlyingBasic.GetPrice(gecko)
				if err != nil {
					return err
				}
